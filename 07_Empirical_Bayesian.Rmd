...
Рассмотрим пример данных из статьи [@daniel19], в которой аннализировались интервью с людьми из деревени Михалёвская и анализировали ряд консервативных и инновативных  черт в их речи.
...
mikhalevskaja %>% 
  ...
  mutate(alpha_prior = beta_est$estimate[1]*10,
         beta_prior = beta_est$estimate[1]*10,
         alpha_post = conservative+alpha_prior,
         beta_post  = innovative+beta_prior,
         mean_post = alpha_post/(alpha_post+beta_post),
         ratio = conservative/total) %>% 
