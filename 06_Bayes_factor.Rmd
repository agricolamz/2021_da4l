...
tibble(x = seq(0, 1, 0.001),
  ...
  geom_line(aes(y = prior), color = "red")+
  geom_line(linetype = 2, color = "yellow")
```
...
В данном занятии мы рассмотрим только слычай двух модели, но можно рассматривать и случаи, когда моделей много. Посмотрим на соотношение апосториорных распределений двух моделей:
...
В результате получается, что коэффициент Байеса -- это соотношение предельных правдоподобий (числитель теоремы Байеса):
...
# Bayes factor
first_update$product[1]/first_update$product[2]
```

Отметим, что так как мы принимаем, что наши модели равновероятны, то байесовский коэфициент не отличается от частоного фукнций правдоподобия:

```{r}
first_update$likelihood[1]/first_update$likelihood[2]
```
...
# Bayes factor
second_update$product[1]/second_update$product[2]
```
...
library(checkdown)
check_question(round(integrate(m1, 0, 1)[[1]]/integrate(m2, 0, 1)[[1]], 3))
```
