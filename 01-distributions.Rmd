...
```{block, type = "rmdtask"}
На графике представлены предсказания трех Пуассоновских моделей, какая кажется лучше?
```
...
```{block, type = "rmdtask"}
Какая из логнормальных моделей для длительности гласных американского английсокго из [@hillenbrand95] лучше подходит к данным? Попробуйте самостоятельно построить данный график.
```
...
vowels %>% 
  ...
  labs(subtitle = "синяя: log μ = 5.487, log σ = 0.262\nкрасная: log μ = 5.687, log σ = 0.342\nзеленая: log μ = 5.487, log σ = 0.262\n")
```
...
В интернете много ресурсов, но вот еще есть [вот этот](http://www.math.wm.edu/~leemis/chart/UDR/UDR.html).
