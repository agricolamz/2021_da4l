...
vowels <- read_csv("https://raw.githubusercontent.com/agricolamz/2021_da4l/master/data/Coretta_2017_icelandic.csv")
ph <- lingtypology::phoible.feature()

ph %>% 
  group_by(inventoryid, language, glottocode) %>% 
  count(segmentclass) %>% 
  pivot_wider(names_from = segmentclass, values_from = n) %>% 
  select(-tone) %>% 
  mutate(area = lingtypology::area.lang(language)) ->
  res
...
res_2 %>% 
  ggplot(aes(consonant, vowel))+
  geom_point()+
  facet_wrap(~area)+
  geom_smooth()

vowels %>% 
  filter(aspiration == "yes", vowel == "e") %>% 
  ggplot(aes(word.dur, closure.dur))+
  geom_point()+
